...
def createDigitLabel(y,labels=None):
    if(labels is None):
        labels = sorted(numpy.unique(y))
        return numpy.array([labels.index(i) for i in y]),labels
    else:
        return numpy.array([labels.index(i) for i in y])
...
def test_SdA(X_train,X_test,Y_train,Y_test,network,finetune_lr=0.1, pretraining_epochs=300,
             pretrain_lr=0.001, training_epochs=20, batch_size=1,corruption_value=0.0,outputfile='result.txt'):
    """
    ...
    # X_train = np.load('X_train_stats_numeric.npy')
    # X_test = np.load('X_test_stats_numeric.npy')
    ...
    Y_train,labels = createDigitLabel(np.load(Y_train))
    Y_test = createDigitLabel(np.load(Y_test),labels)
    # print(Y_test)
    # sys.exit()
    ...
    X_trainF,X_validF,Y_trainF,Y_validF = train_test_split(X_train,Y_train,test_size=0.1)
    ...
    train_set_x_all, train_set_y_all =  shared_dataset((X_train,Y_train))

    datasets = [(train_set_x,train_set_y),(valid_set_x,valid_set_y),(test_set_x,test_set_y)]

    dmat = int(np.sqrt(X_train.shape[1]))
    ...
    print(
        ...
        % (best_validation_loss * 100., best_iter + 1, test_score * 100.), file=open(outputfile,'w')
    )
    ...
    np.save('test_final.npy',np.array(sda.finaloutput(test_set_x)))
...
def createfig():
    ...
    dmat = int(np.sqrt(w.shape[0]))
    image = Image.fromarray(tile_raster_images(
            X=w.T,
            img_shape=(dmat, dmat), tile_shape=(10, 10),
            tile_spacing=(1, 1)))
    image.save('myfilters.png')
